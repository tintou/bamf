...
static GString *desktop_file_hint_for_window (WindowMatcher * self,
					      WnckWindow * window);
static GString *exec_string_for_window (WindowMatcher * self,
					WnckWindow * window);
static GString *exec_string_for_desktop_file (WindowMatcher * self,
					      GString * desktopFile);
static GString *get_open_office_window_hint (WindowMatcher * self,
					     WnckWindow * window);

static gboolean is_open_office_window (WindowMatcher * self,
				       WnckWindow * window);

static void process_exec_string (WindowMatcher * self, GString * execString);
static void handle_window_opened (WnckScreen * screen, WnckWindow * window,
				  gpointer data);
static void handle_window_closed (WnckScreen * screen, WnckWindow * window,
				  gpointer data);
...
static void
profile_start (const char *description)
{
...
static void
profile_stop (const char *description)
{
  ...
  if (desktopFile == NULL || desktopFile->len == 0)
      ...
      if (exec != NULL)
	{
	  ...
	  if (exec->len > 0)
	    {
	      desktopFile =
		g_hash_table_lookup (priv->desktop_file_table, exec);
	    }
					     ...
					     GString * desktopFile)
{
  ...
  while (glist_item != NULL)
	...
	{
	  GString *windowExec =
	    exec_string_for_window (self, glist_item->data);
...
static GString *
desktop_file_hint_for_window (WindowMatcher * self, WnckWindow * window)
{
...
exec_string_for_desktop_file (WindowMatcher * self, GString * desktopFile)
{
  GDesktopAppInfo *desktopApp =
    g_desktop_app_info_new_from_filename (desktopFile->str);
  ...
  while (parts[i] != NULL)
      ...
      if (!g_str_has_prefix (part, "-"))
	{
	  ...
	  for (j = 0; j < self->priv->bad_prefixes->len; j++)
	    {
	      GRegex *regex =
		g_array_index (self->priv->bad_prefixes, GRegex *, j);
...
is_open_office_window (WindowMatcher * self, WnckWindow * window)
{
  ...
  return (group
	  && g_str_has_prefix (wnck_class_group_get_name (group),
			       "OpenOffice"));
}
...
static void
set_window_hint (WindowMatcher * self, WnckWindow * window)
{
  ...
  XChangeProperty (XDisplay,
		   ...
		   8,
		   PropModeReplace,
		   (unsigned char *) desktopFile->str, desktopFile->len);
  XCloseDisplay (XDisplay);
}
