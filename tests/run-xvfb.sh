...
if [ -z $DISPLAY ] && [ -n "$XVFB_PATH" ]; then

if [ -n "x$LOG_PATH" ]; then
xvfb_log=$LOG_PATH/Xvfb.out
else
xvfb_log=/dev/null
fi

$XVFB_PATH -ac -noreset -screen 0 800x600x16 -help 2>/dev/null 1>&2
...
{ $XVFB_PATH -ac -noreset -screen 0 800x600x16 :$XID -screen 0 800x600x16 -nolisten tcp -auth /dev/null > $xvfb_log 2>&1 & trap "kill -15 $! || true" 0 HUP INT QUIT TRAP USR1 PIPE TERM ; } || { echo "Gtk+Tests:ERROR: Failed to start Xvfb environment for X11 target tests."; exit 1; }
DISPLAY=:$XID
export DISPLAY
fi
